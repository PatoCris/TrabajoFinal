 ...
 * @author Asus
 */
public class GestionJefeTaller extends javax.swing.JInternalFrame {
    private ControladoraVista cv;
    private String bandera;
    private DefaultTableModel miTabla;
    private UtilVista util;
    /**
     * Creates new form GestionJefeTaller
     */
    public GestionJefeTaller(ControladoraVista controladoraVista) throws Exception {
        initComponents();
        cv = controladoraVista;
        util = new UtilVista();
        cargarTabla(tblJefeTaller, cv.traerJefesTaller(true));
        estadoInicio();
    }
    ...
    public void cargarTabla(JTable laTabla, List<JefeTaller> lista) throws Exception {
    ...
    private void initComponents() {
        ...
        tblJefeTaller = new javax.swing.JTable();
        ...
        setTitle("Gestión de Jefe de Taller");
        setName("frmJefeTaller"); // NOI18N

        jLabel1.setFont(new java.awt.Font("Politica", 1, 24)); // NOI18N
        jLabel1.setText("GESTIÓN DE JEFES DE TALLER");
        ...
        tblJefeTaller.setModel(new javax.swing.table.DefaultTableModel(
            new Object [][] {
                {null, null, null, null},
                {null, null, null, null},
                {null, null, null, null},
        ...
        tblJefeTaller.addMouseListener(new java.awt.event.MouseAdapter() {
            public void mouseClicked(java.awt.event.MouseEvent evt) {
                tblJefeTallerMouseClicked(evt);
            }
        });
        jScrollPane1.setViewportView(tblJefeTaller);
    ...
    private void btnGuardarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnGuardarActionPerformed
        ...
        try {
            ...
            if(bandera.equals("nuevo")){
                this.cv.nuevoJefeTaller(dniInt, nombre, apellido, telefono, direccion, cuilInt, true);
                bandera = "";
                estadoInicio();
            }else{
                if(bandera.equals("editar")){
                    int codigo = Integer.valueOf(txtCodigo.getText());
                    this.cv.editarJefeTaller(codigo, dniInt, nombre, apellido, telefono, direccion, cuilInt, true);
                    estadoInicio();
                }
            }
            cargarTabla(tblJefeTaller, cv.traerJefesTaller(true)); //ACTUALIZAMOS LA TABLA
        } catch (Exception ex) {
    ...
    private void tblJefeTallerMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tblJefeTallerMouseClicked
        if(tblJefeTaller.getSelectedRow() != -1){
            txtCodigo.setText(tblJefeTaller.getValueAt(tblJefeTaller.getSelectedRow(), 0).toString());
            txtDni.setText(tblJefeTaller.getValueAt(tblJefeTaller.getSelectedRow(), 1).toString());
            txtNombre.setText(tblJefeTaller.getValueAt(tblJefeTaller.getSelectedRow(), 2).toString());
            txtApellido.setText(tblJefeTaller.getValueAt(tblJefeTaller.getSelectedRow(), 3).toString());
            txtTelefono.setText(tblJefeTaller.getValueAt(tblJefeTaller.getSelectedRow(), 4).toString());
            txtDireccion.setText(tblJefeTaller.getValueAt(tblJefeTaller.getSelectedRow(), 5).toString());
            txtCuil.setText(tblJefeTaller.getValueAt(tblJefeTaller.getSelectedRow(), 6).toString());
        }
    }//GEN-LAST:event_tblJefeTallerMouseClicked
    ...
    private void btnBusquedaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnBusquedaActionPerformed
        if((!txtBusquedaDni.getText().isEmpty()) || (!txtBusquedaApellido.getText().isEmpty())){
            ...
            try {
                cargarTabla(tblJefeTaller, cv.traerJefesTallerBusqueda(true, apellidoBusqueda, dniBusquedaInt));
            } catch (Exception ex) {
                JOptionPane.showMessageDialog(null, ex.getMessage());
            }
        }else{
    ...
    private void btnEliminarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnEliminarActionPerformed
        try {
            if(tblJefeTaller.getSelectedRow() != -1){
                int seleccion = JOptionPane.showConfirmDialog(null, "¿Está seguro desea eliminar el Jefe de Taller?", "Input", JOptionPane.YES_NO_OPTION);
                if (seleccion == JOptionPane.YES_OPTION) {
                    int codigo = Integer.valueOf(tblJefeTaller.getValueAt(tblJefeTaller.getSelectedRow(), 0).toString());
                    cv.elminarJefeTaller(codigo);
                    cargarTabla(tblJefeTaller, cv.traerJefesTaller(true));
                }
            }else{
                JOptionPane.showMessageDialog(null, "Seleccione un Jefe de taller de la lista.");
            }
        } catch (Exception e) {
    ...
    private javax.swing.JTable tblJefeTaller;
